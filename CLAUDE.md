...
- `发票附件/*.pdf` — direct-print A4 attachments (not invoices, no paste sheet). `process_attachments` rasterizes each page full-A4 and appends them to the **end** of the output, after all invoice/paste-sheet pairs. A blank page is inserted after **every content page** so that under single-sided printing each page of each attachment occupies its own sheet's front side (a 3-page attachment becomes 6 pages: content/blank/content/blank/content/blank). Root path is hardcoded as `attachment_root` in the notebook. Optional — empty/absent folder is silently skipped.
